...
// Explicació per a no-programadors: Importem els fons del mòbil i de fons que usarem a les fases
// @ts-ignore
...
interface PropsWebLandingSmartphone {
...
export default function WebLandingSmartphone({ 
  onEntrarWorkspace, 
  onEntrarBackoffice, 
  onSimularEntrarMovil,
  onAnarMossos
}: PropsWebLandingSmartphone) {
  ...
  // Explicació per a no-programadors: Estat per discernir si el selector d'oposicions tipus desplegable està obert o tancat en el mòbil.
  const [dropdownObers, setDropdownObers] = useState<boolean>(false);
  ...
  return (
    <div className="bg-[#020b16] text-slate-100 min-h-screen font-sans selection:bg-blue-600 selection:text-white transition-colors duration-500 overflow-x-hidden">
      ...
      >
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', zIndex: 10 }}>
            {/* Logo d'OposicionsCatalunya with colors of the flag */}
            <span 
              style={{ 
                fontSize: '22px', 
                fontWeight: '950', // Altament marcat amb estil elegant
                fontStyle: 'italic', // Text cursiva de marca registradora
                letterSpacing: '0.5px',
                color: '#ffffff',
                userSelect: 'none'
              }}
            >
              <span>Oposi</span>
              <span style={{ color: '#FFDF00' }}>CAT</span>
            </span>
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '28px' }}>
            ...
            {/* Botó de Conversió Principal: Descàrrega directa de l'APP per a mòbils */}
            {/* Explicació per a no-programadors: Aquest botó és el cor de la conversió mòbil. Condueix a la descàrrega o accés de l'APP de color blau fluix. */}
            <button
              ...
              style={{
                ...
                boxShadow: '0 4px 14px 0 rgba(96, 165, 250, 0.35)',
              }}
      ...
      {/* Explicació per a no-programadors:
          Aquesta és la nova barra d'oposicions adaptada per a dispositius mòbils (Smartphone).
          Ara, en comptes dels 4 blocs amuntegats, hem creat un botó unificat elegant de color groc que permet desplegar 
          amb comoditat cadascuna de les 4 oposicions existents amagant-les sota un desplegable vertical molt usable.
          Cadascuna de les oposicions manté els colors sol·licitats: blau per a Mossos, vermell per a Bombers, verd per a Agents Rurals i taronja per a Protecció Civil. */}
      <div id="barra-sirenes-superior" className="w-full bg-[#050b14]/95 border-b border-[#111e36] py-4 px-6 relative z-50 select-none flex flex-col items-center">
        <button
          onClick={() => setDropdownObers(!dropdownObers)}
          style={{
          ...
          className="hover:brightness-110 active:scale-95 transition-all flex items-center justify-center gap-1.5 font-sans"
        >
          <span>🔍</span> Sel·lecciona l'oposició {dropdownObers ? '▲' : '▼'}
        </button>
        ...
        <AnimatePresence>
          {dropdownObers && (
            <motion.div
              ...
              className="w-full max-w-[380px] overflow-hidden flex flex-col gap-2 relative z-50"
            >
              {/* Opció 1: Mossos d'Esquadra - Blau */}
              <button
                onClick={() => {
                  setDropdownObers(false);
                ...
                className="hover:brightness-110 active:scale-[0.98] transition-all flex items-center justify-between font-sans"
              >
                <span>👮 Mossos d'Esquadra</span>
                <span className="bg-[#10b981] text-white text-[8px] px-1.5 py-0.5 rounded font-black whitespace-nowrap">● Comencem!</span>
              </button>

              {/* Opció 2: Bombers de Catalunya - Vermell */}
              <button
                onClick={() => {
                  setDropdownObers(false);
              ...
              >
                ...
                <span className="bg-black/35 text-slate-350 text-[8px] px-1.5 py-0.5 rounded font-black whitespace-nowrap flex items-center gap-0.5">🔒 Pròximament</span>
              </button>

              {/* Opció 3: Agents Rurals - Verd */}
              <button
                onClick={() => {
                  setDropdownObers(false);
              ...
              {/* Opció 4: Protecció Civil - Taronja */}
              <button
                onClick={() => {
                  setDropdownObers(false);
            ...
            <>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-4">
                <button
                  ...
                  className="w-full sm:w-auto px-8 py-4.5 rounded-2xl bg-[#FFDF00] hover:bg-yellow-400 text-slate-950 text-xs font-black italic uppercase tracking-wider shadow-xl shadow-yellow-500/10 transition-all cursor-pointer active:scale-95"
                >
                  Començar a Estudiar Gratuïtament
                </button>
                <a
                  href="#proves"
                  className="w-full sm:w-auto px-8 py-4.5 rounded-2xl bg-slate-900 hover:bg-slate-850 text-white text-xs font-black italic uppercase tracking-wider border border-slate-800 transition-all text-center"
                >
