 ...
 * 5 passos per personalitzar la dieta + resum final.
 */
...
export default function DietaPremiumQuiz({ onTornar }: { onTornar: () => void }) {
  ...
  // Aquesta funció s'encarrega de guardar l'opció triada i marcar el pas com a llest
  const handleSelectOption = (option: string) => {
    // 1. Guardem l'opció seleccionada en l'índex que toca (el pas actual)
    const newSelections = [...selections];
    newSelections[currentStep] = option;
    setSelections(newSelections);

    // 2. "Autocompletem" el botó: fem que el pas es consideri finalitzat automàticament
    ...
    return (
      <div className="fixed inset-0 w-full flex flex-col items-center bg-[#00274d] overflow-y-auto px-6 pb-20">
        <header className="pt-8 w-full flex flex-col items-center gap-4 pb-10">
          <div className="bg-yellow-400 px-6 py-2 rounded-xl shadow-lg shadow-yellow-400/20">
        ...
        <main className="w-full max-w-md flex flex-col gap-8">
          ...
          <div className="flex flex-col gap-4">
            <button 
              ...
              className="w-full bg-emerald-500 text-[#00274d] rounded-2xl py-5 font-black italic uppercase tracking-widest flex items-center justify-center gap-3 shadow-xl shadow-emerald-500/20 active:scale-95 transition-all"
            >
              <Calculator size={20} />
              Calcular la dieta
            </button>
            <button 
              onClick={resetQuiz}
              className="w-full bg-white/5 border border-white/10 text-white/60 rounded-2xl py-5 font-black italic uppercase tracking-widest flex items-center justify-center gap-3 hover:bg-white/10 active:scale-95 transition-all"
            >
              <RotateCcw size={20} />
              Modificar respostes
            </button>
        ...
        <footer className="pt-10">
          <button onClick={onTornar} className="text-white/20 text-[10px] font-black uppercase tracking-widest hover:text-white transition-colors">
    ...
    <div className="fixed inset-0 w-full flex flex-col items-center bg-[#00274d] overflow-y-auto px-6 pb-20">
      ...
      <header className="pt-8 w-full flex flex-col items-center gap-6 pb-8 md:max-w-xl md:mx-auto">
        <div className="flex items-center justify-between w-full">
          ...
          <div className="w-10" /> {/* Spacer per equilibrar */}
        </div>
        
        <div className="flex flex-col items-center gap-1">
          <span className="text-[10px] font-black uppercase tracking-[0.3em] text-yellow-400 shadow-yellow-400/20 italic">
      ...
      <main className="w-full max-w-md md:max-w-lg flex flex-col gap-6">
        ...
        {/* SELECTORS MÉS PETITS */}
        <div className="flex flex-col gap-2.5">
          {currentQ.options.map((opt) => (
            <button
              key={opt}
              onClick={() => handleSelectOption(opt)}
              className={`w-full p-4 rounded-xl border transition-all flex items-center justify-between group active:scale-[0.99] ${
                selections[currentStep] === opt 
                  ? "bg-yellow-400/10 border-yellow-400/50 text-yellow-400 shadow-lg shadow-yellow-400/5" 
        ...
        {/* CONTROL DE COMPLERT / SEGÜENT RECOSTAT */}
        <div className="flex flex-col gap-3 mt-2">
          <button 
            onClick={markAsCompleted}
            disabled={!selections[currentStep] || isStepCompleted[currentStep]}
            className={`w-full py-3 rounded-lg font-black italic uppercase tracking-widest border text-[10px] transition-all ${
          ...
          <button 
            ...
            className={`w-full py-4 rounded-2xl font-black italic uppercase tracking-[0.2em] text-sm flex items-center justify-center gap-2 transition-all ${
              isStepCompleted[currentStep]
                ? "bg-yellow-400 text-[#00274d] shadow-lg shadow-yellow-400/10 active:scale-95"
