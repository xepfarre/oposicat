...
import { ChevronLeft, BarChart3, Target, AlertTriangle, RefreshCw, ChevronDown, ChevronUp, Check, Home, MessageSquare, Bell } from "lucide-react";
...
// Explicació per a no-programadors: Importem la imatge oficial d'OposiTeòrica per aplicar de fons amb transparència
// @ts-ignore
...
export default function ExamensOposimossosInici({ 
  ...
  // Explicació per a no-programadors: Estat per a l'àvatar de l'alumne o alumana desat a la memòria local.
    ...
    const carregarEstadistiquesReals = async () => {
      ...
      try {
        ...
        if (statsSnap.exists()) {
          ...
          // llegim les respostes individuals de l'estudiant per si té històric antic i reconstruïm os totals.
  ...
  return (
    <div className="fixed inset-0 w-full flex flex-col items-center bg-[#010915] overflow-y-auto px-6 pb-32" style={{ WebkitOverflowScrolling: "touch" }}>
      ...
      {/* Explicació per a no-programadors: Capa decorativa d'imatge de fons corporativa en qualitat professional amb el color fosc corporatiu */}
      ...
      <div className="relative z-10 w-full max-w-lg md:max-w-4xl flex flex-col items-center">
        ...
        <main className="w-full flex flex-col gap-8 mt-4">
              ...
              {resetConfirm ? (
                ...
                <button 
                  onClick={() => {
                    ...
                    setTimeout(() => setResetConfirm(false), 4500);
                  }}
          ...
          {tab === 'examen' ? (
                  ...
                  <div key={bloc.id} className="flex flex-col gap-2">
                    ...
                    >
                      ...
                      <button 
                        ...
                        className="flex-1 p-4 flex items-center justify-between transition-all group hover:bg-white/5 text-left cursor-pointer"
                      >
                      ...
                      <div className="flex items-center gap-0">
                        ...
                        <button 
                          ...
                          className={`px-4 h-full flex items-center justify-center text-[10px] font-black uppercase tracking-widest transition-all cursor-pointer ${
                            seleccions[bloc.id].length === bloc.temes.length
                            ? 'bg-yellow-400 text-[#010915]'
                      ...
                      <div className="flex flex-col gap-1.5 px-2 animate-in slide-in-from-top-2 duration-200">
                        {bloc.temes.map(tema => (
                          <button 
                            ...
                            className={`w-full p-3 rounded-xl border flex items-center gap-4 transition-all text-left cursor-pointer ${
                          ...
                          >
                            <div className={`w-6 h-6 flex items-center justify-center rounded-lg border font-black text-[10px] shrink-0 transition-all ${
                              seleccions[bloc.id].includes(tema.id)
                              ? 'bg-yellow-400 text-[#010915] border-yellow-400'
            ...
            <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-2 duration-300 font-sans">
              ...
              <button 
                onClick={() => onComencar(20, 'inf', {})} // Pràctica d'errades per defecte
      ...
      {/* Comentari planer per a no-programadors: Barra corporativa de navegació persistent adaptada elegantment */}
      {onAnarSeccio && (
        <div 
