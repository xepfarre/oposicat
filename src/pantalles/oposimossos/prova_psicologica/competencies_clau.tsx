    ...
    <div className="flex flex-col flex-1 gap-6 w-full animate-in fade-in duration-500 pb-20">
      ...
      <div className="bg-[#1a3a5a]/30 border border-white/10 rounded-[2rem] pt-6 px-6 pb-8 -mx-4 shadow-2xl backdrop-blur-sm">
        <div className="flex flex-col items-center gap-4 text-center">
          ...
          <h3 className="text-white font-[900] italic leading-tight uppercase text-[12px] tracking-widest px-4">
          ...
          <button className="mt-2 w-full max-w-[220px] bg-cyan-400/10 hover:bg-cyan-400/20 border border-cyan-400/40 rounded-xl py-3 px-6 flex items-center justify-center gap-2 transition-all active:scale-95 group">
            <Info size={16} className="text-cyan-400" />
            <span className="text-cyan-400 font-black italic uppercase text-[10px] tracking-widest">DESCOBREIX COM S'AVALUA</span>
          </button>
        </div>
      </div>
      ...
      <div className="flex flex-col items-center gap-2">
        ...
        <span className="text-[10px] text-white/40 font-black uppercase tracking-[0.3em] italic text-center">
        ...
        {competencies.map((comp, idx) => (
          <div key={idx} className="flex flex-col gap-3">
             <div className="flex items-center gap-3 px-2">
                ...
                <h4 className="text-[11px] font-black italic uppercase text-cyan-400/80 tracking-widest whitespace-nowrap">
                  {comp.titol}
                </h4>
                <div className="h-[1px] flex-1 bg-white/5" />
             </div>
             
             <div className="bg-[#1a3a5a]/20 border border-white/5 rounded-[1.5rem] p-5 shadow-inner">
                <ul className="flex flex-col gap-3">
                  {comp.punts.map((punt, pIdx) => (
                    <li key={pIdx} className="flex gap-3 items-start group">
                      <CheckCircle2 size={14} className="text-cyan-400/40 mt-0.5 shrink-0" />
                      <p className="text-[11px] text-white/70 leading-relaxed italic group-hover:text-white/90 transition-colors">
                        {punt}
                      </p>
                    </li>
                  ))}
