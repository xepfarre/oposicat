...
export default function ProvaPsicologicaInici({ onTornar }: { onTornar: () => void }) {
  return (
    ...
    <div className="flex h-screen w-full flex-col items-center pb-6 px-10 bg-[#00274d] overflow-y-auto">
      ...
      {/* 
          CONTINGUT PRINCIPAL: 
          Disseny adaptat a tauletes (md:grid-cols-2).
      */}
      <main className="w-full max-w-sm md:max-w-2xl flex flex-col items-center flex-1 py-2 md:py-4">
        <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-2 md:gap-3">
          ...
          {/* Bloc 3: Preparació */}
          <button className="w-full md:col-span-2 bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/30 rounded-xl py-6 md:py-10 text-emerald-100 font-black italic uppercase text-[11px] md:text-sm tracking-widest transition-all active:scale-95 shadow-lg shadow-emerald-900/20">
            Prepara't les proves
          </button>
